...
export default function FormulaSection() {
  ...
  return (
    <div className="relative w-full min-h-[125vh] flex items-center justify-center bg-black overflow-hidden select-none">
      
      {/* Dynamic Ambient Orbit Glows in CSS */}
      <div className="formula-ambient absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-to-tr from-[var(--accent)]/10 to-[#00e5ff]/5 rounded-full blur-[180px] pointer-events-none animate-pulse duration-[6000ms]" />
      <div className="formula-ambient absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[350px] h-[350px] bg-white/[0.01] border border-white/[0.04] rounded-full pointer-events-none scale-100 animate-ping duration-[8000ms]" />
      <div className="formula-ambient absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-white/[0.005] border border-dashed border-white/[0.08] rounded-full pointer-events-none" />
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 py-24 md:px-12 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center h-full">
        ...
        {/* Left Elements (Cols 1-4) - Fly in from Left; z above fixed WebGL for readable callouts */}
        <div className="relative z-[30] lg:col-span-4 flex flex-col space-y-8 lg:space-y-16 lg:text-right items-start lg:items-end">
          {leftFormula.map((item, index) => (
            <motion.div
              key={item.label}
              className="formula-callout group space-y-3 max-w-md lg:pr-8 border-l-2 lg:border-l-0 lg:border-r-2 border-[var(--accent)]/30 pl-6 lg:pl-0 pr-0 lg:pr-6"
        ...
        {/* Right Elements (Cols 9-12) - Fly in from Right; z above fixed WebGL for readable callouts */}
        <div className="relative z-[30] lg:col-span-4 flex flex-col space-y-8 lg:space-y-16 items-start">
          {rightFormula.map((item, index) => (
            <motion.div
              key={item.label}
              className="formula-callout group space-y-3 max-w-md lg:pl-8 border-l-2 border-[#00e5ff]/30 pl-6"
              initial={{ opacity: 0, x: 100 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.8, delay: index * 0.2 }}
